          ...
          <div class="row">
            <div class="col-md-12">
              <div class="btn-group pull-right" id="templatemo_sort_btn">
                <form class="navbar-form" action="Pendding_DS.php" method="GET">
              ...
              <div class="table-responsive">
                <h4 class="margin-bottom-15">Pending Table</h4>
...
//設定SQL語法
$d=date('Y/m/d');
$sqlstr = "SELECT * FROM order_data where status='Pending' ORDER BY no DESC"; //修改成你要的 SQL 語法
...
<td>{$data['address']}</td>
<td>{$data['phone']}</td>
<td><a href='editStatus_O.php?id=".$data['no']."&userid=".$username."' class='btn btn-default' >Delivery Pending</a></td>
...
echo $main;
echo "</table> </div>";
echo "<br /><a href=Order.php?page=1&id=$username>最前</a> ";
echo "第 ";
for( $i=1 ; $i<=$pages ; $i++ ) {
 if ( $page-3 < $i && $i < $page+3 ) {
  echo "<a href=?page=".$i."&id=$username>".$i."</a> ";
 }
 //if ( $i % 5 == 0 ) echo "<br />";
} //分頁頁碼
echo " 頁 <a href=?page=".$pages."&id=$username>最後</a><br /><br />";
