...
//設定SQL語法
$m=$_GET['orderDate'];
$sqlstr = "SELECT * FROM order_data where date='".$m."' ORDER BY no ASC"; //修改成你要的 SQL 語法
...
<td><a href='editOrder.php?id=".$data['no']."&userid=$userid' class='btn btn-default' >Edit</a></td>                    
...
echo $main;
echo "</table>";
echo "<br /><a href=Order_Search.php?page=1&orderDate=".$m."&userid=$userid>最前</a> ";
echo "第 ";
for( $i=1 ; $i<=$pages ; $i++ ) {
 if ( $page-3 < $i && $i < $page+3 ) {
  echo "<a href=?page=".$i."&orderDate=".$m."&userid=$userid>".$i."</a> ";
 }
 //if ( $i % 5 == 0 ) echo "<br />";
} //分頁頁碼
echo " 頁 <a href=?page=".$pages."&orderDate=".$m."&userid=$userid>最後</a><br /><br />";
